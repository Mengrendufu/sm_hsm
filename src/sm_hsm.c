                               ...
                               SM_StatePtr target) SM_HSM_RETT
{
    ...
    // Exit curr → source (exclusive).  Shared by all transition types.
    s = me->curr;
    while (s != source) {
        if (s == (SM_StatePtr)0) {
            SM_ERROR("Punishment.");
    ...
    if (source == target) {
        ...
        // (b) source is target's parent: no exit source, enter target.
        i = 1;
    } else if (
        ((SM_StatePtr)(source))->super == ((SM_StatePtr)(target))->super
    ) {
